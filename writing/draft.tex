...
Regardless of where the carrier resides, the physical channel structure is always the same. It is based on LTE, but scaled down to accommodate for the reduced need for high transmission throughput. That means the carrier bandwidth is narrower, but preserving the ratio between signali resource blocks and transmission blocks.

NB-IoT downlink is defined to use 12 sub-carriers, where each sub-carrier has bandwidth of 15 kHz. The uplink uses either a single sub-carrier, which can be of varying bandwidth: 3,75 kHz or 15 kHz, or it uses 3, 6, or 12 sub-carriers of 15 kHz bandwidth each\cite{3gpp}.

The total bandwidth of the downlink sums to 180 kHz. That fits neatly into a GSM transmission channel (TCH) which is 200 kHz wide. The LTE guard band varies in width, depending on carrier bandwidth. Most deployments of LTE though, use 5 MHz or larger bandwidth, since the smaller bandwidths introduce exceptions from the normal structure.
